...
import com.aqi.utils.MatrixDatePicker;
...
public class UpdateHealthProfileController {
    ...
    public void initialize() {
        // Initialize your custom date picker
        new MatrixDatePicker(dobField, dobButton);
        ...
        // Listen to the TextField text to calculate age
        dobField.textProperty().addListener((obs, oldText, newText) -> {
            if (newText != null && !newText.trim().isEmpty()) {
                try {
                    LocalDate dob = LocalDate.parse(newText.trim(), dateFormatter);
                    ageDisplayLabel.setText("Age: " + Period.between(dob, LocalDate.now()).getYears() + " yrs");
        ...
        backButton.setOnAction(e -> SceneManager.switchScene("/views/ViewProfile.fxml", "View Profile"));
    ...
    private void loadExistingProfile() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                Platform.runLater(() -> {
                    if (dob != null)       dobField.setText(dateFormatter.format(dob.toLocalDate()));
                    ...
                    if (updatedAt != null) {
                        ...
                        lastUpdatedLabel.setText("Last updated: " + dateFormatter.format(d) + " at " + time);
                    }
                });
            }
    ...
    private void handleUpdate() {
        ...
        new Thread(() -> {
            ...
            try (Connection conn = DatabaseConnection.getConnection();
                ...
                // Parse the text field back into a LocalDate for the database
                LocalDate dob = LocalDate.parse(dobField.getText().trim(), dateFormatter);
                ...
                Platform.runLater(() -> showAlert("Failed to update: " + e.getMessage()));
                e.printStackTrace();
            }
        }).start();
    }
    ...
    private boolean validateInputs() {
        if (dobField.getText() == null || dobField.getText().trim().isEmpty()) return false;
    ...
    private void showAlert(String msg) {
        ...
        a.setTitle("Validation Error"); a.setHeaderText(null); a.setContentText(msg); a.showAndWait();
    }
    ...
    private void hideSuccess() {
        FadeTransition f = new FadeTransition(Duration.millis(250), successBox);
        f.setFromValue(1); f.setToValue(0);
        f.setOnFinished(e -> { successBox.setVisible(false); successBox.setManaged(false); successBox.setOpacity(1); successBox.setScaleX(1); successBox.setScaleY(1); successBox.setTranslateY(0); });
        f.play();
    }
}